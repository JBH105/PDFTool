...
export default function Home() {
  return (
    ...
    <div className="w-full max-w-[1080px] block md:flex justify-between items-center">
      <div className="w-[100%] md:w-[50%]">
        <p className="text-[32px] md:text-[44px] text-[#00308F] font-bold leading-[45px] md:leading-[55px] text-center md:text-left">Best E-Commerce Label Crop
          and PDF Tool Provider</p>
        <p className="text-[18px] md:text-[22px] text-center md:text-left pt-2 pb-4">Get access to premium pdf tools</p>
        <div className="flex justify-center md:justify-start">
          <button className="border-2 border-[#002D62] hover:bg-[#002D62] hover:text-[#fff] px-2 md:px-4 py-2 md:py-3 rounded-xl">
            Explore All PDF Tools
          </button>
        </div>
      </div>
      <div className="w-[100%] md:w-[50%]">
        <img className="w-full h-full min-h-[50vh]" src="/assets/hero_img.jpg" alt="" />
      </div>
    </div>
  </section>
  ...
  <section className="w-full flex justify-center py-4 px-4 bg-[#7CB9E8] bg-opacity-20">
    <div className="w-full max-w-[1080px] py-8">
      <p className="text-center text-[#00308F] text-[24px] md:text-[30px] font-bold pb-5">What exactly is great about pdfcroppers that can provide you?</p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {posts.map((post) => (
          <div className="bg-[#fff] rounded-xl flex flex-col items-center p-5" key={posts.id}>
            <img className="w-[50px]" src={post.img} alt="" />
            <p className="text-[20px] text-[#002D62] font-semibold pt-5 pb-2 text-center">{post.title}</p>
            <p className="text-[14px] md:text-[16px] text-center leading-[20px]">{post.des}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  <section className="w-full flex justify-center py-4 px-4">
    <div className="w-full max-w-[1080px] py-8">
      <p className="text-center text-[#00308F] text-[24px] md:text-[30px] font-bold pb-5">What Else You Can Do</p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {works.map((work) => (
          <div className="flex flex-col items-center p-5" key={works.id}>
            <img className="w-[40px]" src={work.img} alt="" />
            <p className="text-[20px] text-[#002D62] font-semibold pt-5 pb-2 text-center">{work.title}</p>
            <p className="text-[14px] md:text-[16px] text-center leading-[20px]">{work.des}</p>
          </div>
        ))}
      </div>
    </div>
